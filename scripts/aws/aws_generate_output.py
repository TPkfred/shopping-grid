...
Note that this script is designed to run on aggregated, processed
data derived from the "datalake" format of raw eStreaming data in 
AWS.

Notes for updating to Lead Price Summaries:
- remove shop_counts_threshold
"""
...
def filter_ratio_mean(df, ratio_mean, shop_counts_threshold):
...
# select most recent
w = (Window
        .partitionBy('market', 'out_departure_date', 'in_departure_date')
        .orderBy(F.desc("search_date"))
        )
