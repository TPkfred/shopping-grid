...
def preprocess_data(df, min_days_til_dept, max_days_til_dept,
    ...
    df_proc = (df
        .withColumn("market", 
            F.concat_ws("-", F.col("origin"), 
                                F.col("destination"))
                    )
    #     .withColumnRenamed("price", "min_fare")
    ...
    # Summary data has already been filtered for min/max days til departure & LOS
    # Besides, we can filter on them if needed, downstream in other uses of this data
    # df_filt = (df_proc
...
def load_process_write_data(date, spark):
    ...
    m = date.month
    d = date.day
    df = spark.read.parquet(BASE_INPUT_DIR).filter(
