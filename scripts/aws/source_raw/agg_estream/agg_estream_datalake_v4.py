...
- v4:
    ...
    - add in flight info
- v3:
    - limit to when request and response PTC are adult
        - saw an instance of an INF in the request being returned as an ADT; 
            the fare was $4.50 for JFK-LHR
- "poc" v2
    - fix shop counts
...
BASE_OUTPUT_DIR = "s3://tvlp-ds-users/kendra-frederick/shopping-grid/agg-raw-data_v4"
...
info_cols_raw = [
    'pcc', 'gds',
    'shop_req_timeStamp', # added in v4
...
def explode_fare_filter_adt(df):
    ...
    df_expl = (df
        ...
        .select("*", F.posexplode("response_PTC").alias("resp_pos", "resp_ptc_"))
...
def preprocess_data(df):
    ...
    df_mod = filter_top_markets(df_mod) # df gets repartitioned here
    df_mod = filter_other(df_mod)
    df_mod = explode_fare_filter_adt(df_mod)
    # df_mod = extract_features(df_mod)
    return df_mod
